# CLAUDE.md
勤怠管理Webアプリ。Laravel 12 / React 19 / TypeScript / Inertia.js / Tailwind / MySQL

## コマンド
```bash
composer setup    # 初回セットアップ
composer dev      # 開発サーバー起動
php artisan test  # テスト
./vendor/bin/pint # フォーマット
```

## 構造
Controller → Service → Repository → Model の厳格な階層。

## 重要ルール
- **マイグレーション作成禁止** — スキーマは `docs/tables/README.md` 参照。変更前にユーザー確認。
- **`Carbon::now()` 禁止** — `CarbonImmutable::now()` を使う。
- **`DB::table()` 禁止** — Eloquent か Repository を使う。
- 規約: `docs/backend/CODING_GUIDE.md`
===
