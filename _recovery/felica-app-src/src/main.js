...
// 5 秒に延長したが、それでも通常の1回タップで表示される事象が
// 報告された。原因はサーバ側のクールダウン（既定10秒、
// config('attendance.felica_stamp_cooldown_seconds')）より本デバウンスが
// 短いこと。5〜10秒の間に発火した重複イベントはここでは吸収されず
// サーバへ送られ、サーバのクールダウンに弾かれてトーストが出ていた。
// サーバ側の秒数と揃えることでこの隙間をなくす。
const CARD_READ_DEBOUNCE_MS = 10000;
